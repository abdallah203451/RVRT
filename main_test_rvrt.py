...
def main():
    ...
    parser.add_argument('--tile_overlap', type=int, nargs='+', default=[2,10,10],
                        help='Overlapping of different tiles')
    parser.add_argument('--num_workers', type=int, default=4, help='number of workers in data loading')
    parser.add_argument('--save_result', action='store_true', help='save resulting image')
    args = parser.parse_args()
    ...
    for idx, batch in enumerate(tqdm(test_loader, desc='Testing videos')):
        lq = batch['L'].to(device)
        folder = batch['folder']
        gt = batch['H'] if 'H' in batch else None
...
def prepare_model_dataset(args):
    ...
    if os.path.exists(f'{args.folder_lq}'):
        ...
        print(f'Dataset not found at {args.folder_lq}. Please check your folder path.')
    return model
...
def test_video(lq, model, args):
    ...
    if num_frame_testing:
        ...
        # Progress bar for temporal clip processing
        for d_idx in tqdm(d_idx_list, desc='Processing temporal clips', leave=False):
            lq_clip = lq[:, d_idx:d_idx + num_frame_testing, ...]
            out_clip = test_clip(lq_clip, model, args)
            out_clip_mask = torch.ones((b, min(num_frame_testing, d), 1, 1, 1))
            if not_overlap_border:
...
def test_clip(lq, model, args):
    ...
    if size_patch_testing:
        ...
        # Progress bars for spatial patch processing
        for h_idx in tqdm(h_idx_list, desc='Processing height patches', leave=False):
            for w_idx in tqdm(w_idx_list, desc='Processing width patches', leave=False):
                in_patch = lq[..., h_idx:h_idx + size_patch_testing, w_idx:w_idx + size_patch_testing]
                out_patch = model(in_patch).detach().cpu()
                out_patch_mask = torch.ones_like(out_patch)
                if not_overlap_border:
